...
class TestManipulabilityTask(unittest.TestCase):
    ...
    def test_task_repr(self):
        ...
        task = ManipulabilityTask(self.frame_name, self.configuration.model, cost=1.0)
    ...
    def test_compute_error_value(self):
        ...
        task = ManipulabilityTask(
            self.frame_name, self.configuration.model, cost=1.0, manipulability_rate=manipulability_rate
        )
        e = task.compute_error(self.configuration)
    ...
    def test_unit_cost_qp_objective(self):
        ...
        task = ManipulabilityTask(self.frame_name, self.configuration.model, cost=1.0, lm_damping=0.0)
        J = task.compute_jacobian(self.configuration)
        e = task.compute_error(self.configuration)
        H, c = task.compute_qp_objective(self.configuration)
    ...
    def test_zero_cost_disables_task(self):
        ...
        task = ManipulabilityTask(self.frame_name, self.configuration.model, cost=0.0)
        J = task.compute_jacobian(self.configuration)
        H, c = task.compute_qp_objective(self.configuration)
    ...
    def test_mask_position(self):
        ...
        task = ManipulabilityTask(self.frame_name, self.configuration.model, cost=1.0, mask="position")
        self.assertTrue(
    ...
    def test_mask_orientation(self):
        ...
        task = ManipulabilityTask(
            self.frame_name, self.configuration.model, cost=1.0, mask="orientation"
        )
        self.assertTrue(
    ...
    def test_mask_planar_xy(self):
        ...
        task = ManipulabilityTask(self.frame_name, self.configuration.model, cost=1.0, mask="planar_xy")
        self.assertTrue(
    ...
    def test_custom_mask(self):
        ...
        task = ManipulabilityTask(self.frame_name, self.configuration.model, cost=1.0, mask=custom_mask)
    ...
    def test_invalid_mask_string_raises(self):
        ...
        with self.assertRaises(ValueError):
            ManipulabilityTask(self.frame_name, self.configuration.model, cost=1.0, mask="invalid_mask")

    def test_invalid_mask_shape_raises(self):
        """Custom mask with wrong shape should raise ValueError."""
        with self.assertRaises(ValueError):
            ManipulabilityTask(
                self.frame_name, self.configuration.model, cost=1.0, mask=np.array([1, 1, 1])
            )
    ...
    def test_invalid_mask_values_raises(self):
        ...
        with self.assertRaises(ValueError):
            ManipulabilityTask(
                self.frame_name, self.configuration.model, cost=1.0, mask=np.array([1, 2, 1, 0, 0, 0])
            )
    ...
    def test_different_manipulability_rates(self):
        ...
        task1 = ManipulabilityTask(
            self.frame_name, self.configuration.model, cost=1.0, manipulability_rate=0.1
        )
        task2 = ManipulabilityTask(
            self.frame_name, self.configuration.model, cost=1.0, manipulability_rate=0.5
        )
    ...
    def test_lm_damping_effect(self):
        ...
        task_with_damp = ManipulabilityTask(
            self.frame_name, self.configuration.model, cost=1.0, lm_damping=1e-3
        )
    ...
    def test_gain_effect(self):
        ...
        task_full = ManipulabilityTask(
            self.frame_name, self.configuration.model, cost=1.0, gain=1.0, manipulability_rate=0.1
        )
        task_half = ManipulabilityTask(
            self.frame_name, self.configuration.model, cost=1.0, gain=0.5, manipulability_rate=0.1
        )
    ...
    def test_cost_scaling(self):
        ...
        task1 = ManipulabilityTask(self.frame_name, self.configuration.model, cost=1.0, lm_damping=0.0)
        task2 = ManipulabilityTask(self.frame_name, self.configuration.model, cost=2.0, lm_damping=0.0)
